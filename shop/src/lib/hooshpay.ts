...
import { db } from "./db";
import { activeGateways, gatewayById, validHooshpaySignature, verifyWithGateway } from "./payments";
import { completePaidOrder } from "./orders";

/**
 * وب‌هوک هوش‌پی.
 *
 * دو مسیر به این تابع می‌رسند:
 *   - `/api/pay/hooshpay/<کد سفارش>` که خودِ سایت روی هر فاکتور می‌فرستد
 *   - `/api/pay/hooshpay` که «آدرس وب‌هوک پیش‌فرض» پنل هوش‌پی است و کد سفارش
 *     ندارد؛ آنجا سفارش از روی `order_id` (یا شمارهٔ فاکتور) پیدا می‌شود
 *
 * در هر دو حالت، امضا بررسی می‌شود و تصمیم نهایی با تأیید سرور‌به‌سرور خود
 * هوش‌پی گرفته می‌شود، نه با محتوای همین درخواست.
 */
export async function handleHooshpayWebhook(
  req: NextRequest,
  code?: string,
  ...
  const orderCode = code || String(payload.order_id ?? "");
  const invoice = String(payload.invoice ?? "");

  const order =
    (orderCode ? await db.order.findUnique({ where: { code: orderCode } }) : null) ??
    (invoice ? await db.order.findFirst({ where: { gatewayRef: invoice } }) : null);
  ...
  const ref = order.gatewayRef || invoice;
